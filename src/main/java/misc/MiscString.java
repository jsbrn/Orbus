...
public class MiscString {
    ...
    /**
     * Wraps the given text. Credit to progcookbook.com's Robert Hanson. I modified it slightly
    ...
    public static String[] wrap(String text, int len, org.newdawn.slick.Font font) {
        ...
        if (word.length() > 0) {
          if (font.getWidth(line.toString())
                  +font.getWidth(word.toString()) > len) { // TODO: StringBuffer toString returns the string?
            lines.add(line.toString());
            line.delete(0, line.length());
          }
          line.append(word);
